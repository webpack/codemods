...
- css-loader options are checked before migrating: `importLoaders`, `sourceMap`, and `esModule` are safely dropped (native CSS covers them), while `url`, `import`, `exportType`, or any other semantic option leaves the rule untouched — native CSS cannot replicate them. `modules` blocks migration only when the rule also matches plain `.css` files (that option applies to every matched file, while `css/auto` only treats `*.module.*` names as CSS modules); rules scoped to `.module.css` migrate normally.
